...
namespace spiral_matrix {
    ...
    auto check_right_square(const matrix_t& matrix, std::size_t row, std::size_t col) -> bool {
        return col + 1 < matrix[row].size() and not matrix[row][col + 1];
    }

    auto check_down_square(const matrix_t& matrix, std::size_t row, std::size_t col) -> bool {
        return row < matrix.size() - 1 and not matrix[row + 1][col];
    }

    auto check_left_square(const matrix_t& matrix, std::size_t row, std::size_t col) -> bool {
        return col > 0 and not matrix[row][col - 1];
    }

    auto check_up_square(const matrix_t& matrix, std::size_t row, std::size_t col) -> bool {
        return row > 0 and not matrix[row - 1][col];
    }

    auto fill(matrix_t& matrix, uint32_t value, std::size_t row, std::size_t col, direction path) -> void { 
    ...
    auto spiral_matrix(uint32_t size) -> matrix_t {
        ...
        auto matrix = matrix_t(size, vector_t(size, 0));
