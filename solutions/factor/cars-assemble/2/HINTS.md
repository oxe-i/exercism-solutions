...
- `zero?` (in [`math`][math]) tests whether the speed is `0`.
- `if` takes a boolean and two quotations — `[ "stopped" ] [ "running" ]`
  — and runs the one that matches. Here the speed is consumed by the
  test, so neither quotation needs a `drop`.
