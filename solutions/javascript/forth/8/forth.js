export class Forth {
  ...
  static #stream(...values) {
    if (values.at(-1) === Forth.#END) return values.slice(0, -1);
    return (gn) => Forth.#stream(...values.flat(Infinity), gn);
  }
  ...
  evaluate(expressions) {
    ...
    const endStream = stream(Forth.#END);
    this.#stack = endStream.reduce((acc, exp) => {
      while (typeof exp === "function") exp = exp(acc.pop());
      acc.push(exp);
