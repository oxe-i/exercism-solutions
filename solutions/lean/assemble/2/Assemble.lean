...
private def Program.evaluate (args : Array Int64) (program : Program) : Int64 := Id.run do
  ...
  while h: i < program.ops.size do
    ...
    | [op, dest, src] =>
      ...
      match op with
      | "mov"  => acc := { regs := acc.regs.set (regIdx dest) srcVal }
      | "add"  => acc := { regs := acc.regs.set (regIdx dest) (destVal + srcVal) }
      | "sub"  => acc := { regs := acc.regs.set (regIdx dest) (destVal - srcVal) }
      | "xor"  => acc := { regs := acc.regs.set (regIdx dest) (destVal ^^^ srcVal) }
      | "and"  => acc := { regs := acc.regs.set (regIdx dest) (destVal &&& srcVal) }
      | "or"   => acc := { regs := acc.regs.set (regIdx dest) (destVal ||| srcVal) }
      | "shl"  => acc := { regs := acc.regs.set (regIdx dest) (destVal <<< srcVal) }
      | "shr"  => acc := { regs := acc.regs.set (regIdx dest) (destVal >>> srcVal) }
      | "mul"  => acc := { regs := acc.regs.set (regIdx dest) (destVal * srcVal) }
      | "div"  => acc := { regs := acc.regs.set (regIdx dest) (destVal / srcVal) }
      | "cmp"  => acc := { regs := acc.regs.set (regIdx "rflags") (compareRegs destVal srcVal) }
      | _ => continue
    | [op, dest] =>
      match op with
