...
abbrev Name := String
abbrev Relatives := Std.HashSet Name
abbrev Parent := Name
abbrev Children := List Name
...
partial def traverse 
  ...
  then some degree
    ...
    let nextLayer ← layer.foldM (init := {}) fun m c =>
...
def degreeOfSeparation (familyTree : List (Parent × Children)) (personA personB : Name) : Option Nat := do
