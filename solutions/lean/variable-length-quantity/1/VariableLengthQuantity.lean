...
def encode (integers : Array Nat) : ByteArray :=
  ByteArray.mk (integers.flatMap fun x => Id.run do
    ...
    let mut num := x
    repeat
      let byte := (num &&& mask).toUInt8
      num := num >>> 7
      acc := acc.push (byte ||| highBit)
    until num == 0
    acc := acc.modify 0 (· &&& (~~~highBit)) 
    -- array reversed, first byte is the last in sequence
    return acc.reverse)

def decode (bytes : ByteArray) : Option (Array Nat) := do
  let mut acc := #[0] 
  -- starts non-empty and pops at the end. Makes bit logic easier
