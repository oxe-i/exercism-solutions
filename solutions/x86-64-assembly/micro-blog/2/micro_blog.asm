...
truncate:
    ...
    mov ecx, 1 << 5            
    pdep ecx, ecx, eax  ; we select only the 5° byte
    tzcnt ecx, ecx      ; we check its idx

    vpcmpeqb ymm2, ymm0, ymm1       
    vpmovmskb edx, ymm2 ; bit set on NUL
    tzcnt edx, edx      ; we check its idx

    cmp ecx, edx               
    cmova ecx, edx      ; we get the lowest idx

    mov eax, -1               
    bzhi eax, eax, ecx  ; clears all bits from idx on
    kmovd k3, eax
    vmovdqu8 [rdi]{k3}, ymm0 ; we store only up to idx
    ret
